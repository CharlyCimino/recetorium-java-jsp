...
<jsp:include page="../includes/inicioHTML.jsp">
    <jsp:param name="elTitulo" value="${ingrediente.nombre}" />
</jsp:include>
...
<section class="container">
    <div class="row pt-3">
        ...
        <p class="lead mb-0">Estas son nuestras recetas que incluyen ${ingrediente.nombre}</p>
    </div>
    <hr/>
    <c:choose>
        <c:when test = "${listaDeRecetas != null && !listaDeRecetas.isEmpty()}">
            <div class="row g-4 mb-3 row-cols-1 row-cols-sm-2 row-cols-lg-3 row-cols-xl-4 row-cols-xxl-5" data-masonry='{"percentPosition": true }' >
                <jsp:include page="../recetas/cardReceta.jsp" />
            </div>
        </c:when>
        <c:otherwise>
            <div class="row my-4">
                <div class="col-12">
                    <p class="display-5 text-danger">Ooops! Parece que no hay recetas con ${ingrediente.nombre}...</p>
                </div>
            </div>
        </c:otherwise>
    </c:choose>
